...
public final class DataGridConnectionUtility {
    ...
    private DataGridConnectionUtility() {
        Runtime.getRuntime().addShutdownHook(new Thread() {
            @Override
            public void run() {
                // You can lock here rather than use a ConcurrentHashMap for
                // configurerMap.
            ...
            if (gigaspace == null) {
                ...
                try {
                    ...
                    GridServiceManager esm = admin.getGridServiceManagers()
                            .waitForAtLeastOne();
                    ProcessingUnit pu = esm.deploy(new SpaceDeployment(
                            spaceName).partitioned(instances, backups));
                    pu.waitForSpace();
                    admin.close();
                    space = configurer.space();
                }
                LocalCacheSpaceConfigurer cacheConfigurer = new LocalCacheSpaceConfigurer(
                        space);
